-- Add original_language to movie_embeddings so the recommendation engine
-- can boost or filter films by preferred cinematic origin (e.g. French cinema).

ALTER TABLE public.movie_embeddings
  ADD COLUMN IF NOT EXISTS original_language text;

CREATE INDEX IF NOT EXISTS movie_embeddings_language_idx
  ON public.movie_embeddings(original_language);

-- Drop existing function first because PostgreSQL cannot change return type via CREATE OR REPLACE.
DROP FUNCTION IF EXISTS public.match_movies_for_recommendation(vector,integer,integer[],text,real,text[],text[],integer,uuid);

-- Recreate match_movies_for_recommendation to expose original_language
-- so the edge function can apply language-based boosting in the candidate pool.
