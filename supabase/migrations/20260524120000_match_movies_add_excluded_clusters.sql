-- Add p_excluded_clusters param so rejected taste clusters are filtered in SQL,
-- not just mentioned in the LLM prompt.

DROP FUNCTION IF EXISTS public.match_movies_for_recommendation(vector,integer,integer[],text,real,text[],text[],integer,uuid,text,integer,integer,text[]);
