-- 1. Fix runtime filter: films with NULL runtime now PASS the duration check
--    (previously they were excluded, losing many valid candidates).
--    Films with known runtime > max_duration are still rejected.
--
-- 2. Add performance indexes for the p_user_id NOT EXISTS subquery:
--    - user_item_feedback(user_id) for WHERE uif.user_id = p_user_id
--    - catalog_items(tmdb_id) for WHERE ci.tmdb_id = me.tmdb_id

CREATE INDEX IF NOT EXISTS user_item_feedback_user_id_idx
  ON public.user_item_feedback (user_id);

CREATE INDEX IF NOT EXISTS catalog_items_tmdb_id_idx
  ON public.catalog_items (tmdb_id);
