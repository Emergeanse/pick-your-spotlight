-- Add p_min_popularity param to filter out obscure titles with very low TMDB popularity.
-- Default NULL means no filter applied (backward compatible).

DROP FUNCTION IF EXISTS public.match_movies_for_recommendation(vector,integer,integer[],text,real,text[],text[],integer,uuid,text,integer,integer,text[],text[]);
