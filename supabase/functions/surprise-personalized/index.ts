...
serve(async (req) => {
  ...
  try {
    ...
    const quota = await consumeQuota(auth.user!.id, "recommendation");
    if (!quota.allowed) return quotaExceededResponse("recommendation", quota, corsHeaders);
    const t0 = Date.now();
    ...
    if (duoUserIds.length === 2 && SUPABASE_URL && SUPABASE_SERVICE_ROLE_KEY) {
      try {
        ...
        const { data: feedbackRows } = await sbAdmin
          ...
          .or("action.in.(not_for_me,dislike,seen,like,love),feedback_type.in.(not_for_me,dislike,seen,like,love)")
          .range(0, 9999);
