...
type OnboardingStep = "welcome" | "genres" | "platforms";
const STEPS: OnboardingStep[] = ["welcome", "genres", "platforms"];
...
const Onboarding = () => {
  ...
  const handleFinish = async () => {
    ...
    try {
      ...
      // Navigate to home with activation flag — will trigger the training flow
      navigate("/app", { state: { activateTraining: true } });
        ...
        {step === "welcome" && (
          ...
          >
            ...
            <motion.p
              ...
              className="text-primary/60 text-xs font-sans text-center max-w-xs mb-10"
            >
              2 petites questions, puis on passe aux choses sérieuses.
            </motion.p>
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4 }} className="flex flex-col sm:flex-row gap-3 items-center">
              <Button variant="hero" size="xl" onClick={handleFinish} disabled={saving}>
                {saving ? <Loader2 className="w-4 h-4 animate-spin" /> : (
                  <>
                    {selectedPlatforms.length === 0 ? "Passer" : "Continuer"}
                    <ArrowRight className="w-4 h-4" />
                  </>
                )}
              </Button>
