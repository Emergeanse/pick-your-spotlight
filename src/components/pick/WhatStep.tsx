...
export type WhatOption = "movie" | "tv" | "both";

interface WhatStepProps {
  onSelect: (what: WhatOption) => void;
}

const options: { value: WhatOption; label: string; description: string; icon: React.ElementType }[] = [
  { value: "movie", label: "Un film", description: "Long-métrage", icon: Film },
  { value: "tv", label: "Une série", description: "Série ou documentaire", icon: Tv },
  { value: "both", label: "Peu importe", description: "Surprends-moi", icon: Layers },
];

const WhatStep = ({ onSelect }: WhatStepProps) => {
  const [selected, setSelected] = useState<WhatOption | null>(null);

  const handleSelect = (what: WhatOption) => {
    setSelected(what);
    setTimeout(() => onSelect(what), 300);
  };
